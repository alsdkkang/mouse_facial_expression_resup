...
class Task1TemporalFolds:
    ...
    def __init__(self, version='1.0', num_frames=10, frame_stride=2,
        ...
        # Try loading CSV first (Colab compatibility), then pickle
        csv_path = task1_path / 'dataset_df.csv'
        if csv_path.exists():
        ...
        # Load folds
        # Check for JSON folds first
        json_folds = list(task1_path.glob('fold*.json'))
        if json_folds:
            fold_df = pd.DataFrame({'foldpath': json_folds})
            fold_df['fold_index'] = fold_df.foldpath.apply(lambda x: int(re.match('.*(\d+)', x.stem).group(1)))
            self.fold_type = 'json'
        else:
            folds = list(task1_path.glob('fold*.pkl'))
            fold_df = pd.DataFrame({'foldpath': folds})
            fold_df['fold_index'] = fold_df.foldpath.apply(lambda x: int(re.match('.*(\d+)', x.stem).group(1)))
            self.fold_type = 'pickle'
    ...
    def __iter__(self):
        for idx in range(len(self.fold_df)):
            ...
            if self.fold_type == 'json':
                import json
                with open(row.foldpath, 'r') as fp:
